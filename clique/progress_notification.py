...
Each signal is tagged with a context variable UUID and a user-defined label to support concurrent execution.
...
def _tag_translator(text: str) -> str:
    try:
        ident = _context_ident.get()
    except LookupError:
        ident = str(uuid.uuid4())
        _context_ident.set(ident)
...
class ProgressNotifier:
    ...
    def __init__(self, label: str, length: int) -> None:
        self._tag = _tag_translator(label)
        self._length = length
        self._active = False
        self._position = 0
